...
describe("story image upload action", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("rejects a MIME type outside the storage allow-list", async () => {
    await expect(requestStoryImageUpload("image/gif", 1000)).rejects.toThrow(
      /MIME type/,
    );
  ...
  it("rejects an image above the 10MB bucket limit", async () => {
    await expect(
      requestStoryImageUpload("image/webp", 10 * 1024 * 1024 + 1),
    ).rejects.toThrow(/size/);
    expect(storageMock.createSignedUploadUrl).not.toHaveBeenCalled();
  });

  it("derives a year/UUID path instead of using a supplied filename", async () => {
    const result = await requestStoryImageUpload("image/jpeg", 240_000);
  ...
  it("checks admin access before touching Supabase", async () => {
    authzMock.requireAdmin.mockRejectedValue(new Error("not authorized"));

    await expect(
      requestStoryImageUpload("image/webp", 240_000),

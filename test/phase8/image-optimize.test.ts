...
describe("story image optimizer helpers", () => {
  it("caps landscape, portrait, and square images without changing their ratio", () => {
    expect(targetDimensions(4032, 3024, STORY_IMAGE_MAX_EDGE)).toEqual({
      width: 2400,
      height: 1800,
    });
    expect(targetDimensions(3024, 4032, STORY_IMAGE_MAX_EDGE)).toEqual({
      width: 1800,
      height: 2400,
    });
    expect(targetDimensions(3000, 3000, STORY_IMAGE_MAX_EDGE)).toEqual({
      width: 2400,
      height: 2400,
    });
  });
  ...
  it("never upscales an image whose dimensions are already within the cap", () => {
    expect(targetDimensions(400, 300, STORY_IMAGE_MAX_EDGE)).toEqual({
      width: 400,
      height: 300,
    });
  });

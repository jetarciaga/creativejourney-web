...
- Supabase project; port `migrations/001_inquiries.sql` unchanged (full schema in Appendix B); add a `destinations` table.
...
- `lib/content.ts`'s `destinations` moves to the database; card links finally point at `/contact?destination=<slug>` instead of `/` — which also closes the old P1-12 task (Appendix C).

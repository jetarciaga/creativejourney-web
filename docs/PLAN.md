...
**Merged and live.** PR #9 merged into `main` at `eb8fb90` (`gh pr view 9`: `state: MERGED`, `mergedAt: 2026-08-15T11:00:04Z`, merge commit `dc55da6`/`309ceab` squashed under `eb8fb90`). Vercel's production deploy is up and confirmed by the user against the live site. Stages 0 through 4.6 — the full Next.js rebuild, the logo-derived brand pass, and the `vercel.json` fix — are done, verified, and shipped. **Stage 5 (Supabase + admin) is also done and verified**, built on `feature/supabase-admin`, not yet committed or merged — see the Stage 5 section below for the full verification. Stage 6 (inquiry form) and beyond have not started.
